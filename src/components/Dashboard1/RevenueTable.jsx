...
import { mockRevReports, subCountyNames } from '../../data/mockData';
...
const RevenueTable = ({ filters = {} }) => {
  ...
  // Apply top-level filters to data
  const filteredData = mockRevReports.filter(r => {
  ...
  const streams = [...new Set(filteredData.map(r => r.rev_stream))];

  const getYears = (stream) => {
    const order = [2025, 2026, 2027];
    return [...new Set(filteredData.filter(r => r.rev_stream === stream).map(r => r.calendar_year))].sort((a, b) => order.indexOf(a) - order.indexOf(b));
  };
  ...
  const getMonths = (stream, year) => {
    ...
    return [...new Set(filteredData.filter(r => r.rev_stream === stream && r.calendar_year === year).map(r => r.calendar_month))]
      .sort((a, b) => order.indexOf(a) - order.indexOf(b));
  };

  const getSubStreams = (stream, year, month) =>
    [...new Set(filteredData.filter(r => r.rev_stream === stream && r.calendar_year === year && r.calendar_month === month).map(r => r.rev_sub_stream))];

  const getStatus = (stream, year, month) => {
    const rec = filteredData.find(r => r.rev_stream === stream && r.calendar_year === year && r.calendar_month === month);
  ...
  return (
    <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      <div style={{ background: 'var(--dark)', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: 'white', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '1px', opacity: 0.7 }}>TABLE: rev_reports</span>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {streams.map(stream => {
              const streamTotal = getAmount(filteredData, { rev_stream: stream });
              ...
              return (
                <React.Fragment key={stream}>
                  ...
                  >
                    {[
                      <td key="name" style={{ padding: '13px 16px', background: hoveredRow === rowId ? '#d8f0e4' : 'var(--green-pale)', borderBottom: '1px solid #c8dfd2', fontFamily: 'Syne, sans-serif', fontWeight: 700, color: 'var(--green)', transition: 'background 0.15s' }}>
                      ...
                      <td key="fy" style={{ padding: '13px 16px', background: hoveredRow === rowId ? '#d8f0e4' : 'var(--green-pale)', borderBottom: '1px solid #c8dfd2', fontWeight: 600, transition: 'background 0.15s' }}>FY2025/2026</td>,
                      <td key="yr" style={{ padding: '13px 16px', background: hoveredRow === rowId ? '#d8f0e4' : 'var(--green-pale)', borderBottom: '1px solid #c8dfd2', transition: 'background 0.15s' }}>—</td>,
                      <td key="all" style={{ padding: '13px 16px', background: hoveredRow === rowId ? '#d8f0e4' : 'var(--green-pale)', borderBottom: '1px solid #c8dfd2', textAlign: 'right', fontFamily: 'DM Mono, monospace', fontWeight: 600, transition: 'background 0.15s' }}>{fmt(streamTotal)}</td>,
                      ...SUB_COUNTIES.map(sc => (
                        <td key={sc} style={{ padding: '13px 16px', background: hoveredRow === rowId ? '#d8f0e4' : 'var(--green-pale)', borderBottom: '1px solid #c8dfd2', textAlign: 'right', fontFamily: 'DM Mono, monospace', transition: 'background 0.15s' }}>
                          {fmt(getAmount(filteredData, { rev_stream: stream, sub_county_id: sc }))}
                        </td>
                      ))
                    ]}
                  </tr>
                  ...
                  {streamExpanded && getYears(stream).map(year => {
                    ...
                    const yearTotal = getAmount(filteredData, { rev_stream: stream, calendar_year: year });
                    ...
                    return (
                      <React.Fragment key={yearKey}>
                        ...
                        <tr onClick={() => toggleYear(yearKey)} style={{ cursor: 'pointer' }} {...rowHover(yearRowId)}>
                          ...
                          {SUB_COUNTIES.map(sc => (
                            <td key={sc} style={{ padding: '11px 16px', background: hoveredRow === yearRowId ? '#f5f9f7' : 'white', borderBottom: '1px solid #e8eeeb', textAlign: 'right', fontFamily: 'DM Mono, monospace', color: '#1a4a7a', transition: 'background 0.15s' }}>
                              {fmt(getAmount(filteredData, { rev_stream: stream, calendar_year: year, sub_county_id: sc }))}
                            </td>
                          ))}
                        </tr>
                        ...
                        {yearExpanded && getMonths(stream, year).map(month => {
                          ...
                          const monthTotal = getAmount(filteredData, { rev_stream: stream, calendar_year: year, calendar_month: month });
                              ...
                              <tr onClick={() => toggleMonth(monthKey)} style={{ cursor: 'pointer' }} {...rowHover(monthRowId)}>
                                ...
                                {SUB_COUNTIES.map(sc => (
                                  <td key={sc} style={{ padding: '9px 16px', background: hoveredRow === monthRowId ? '#f0f7f3' : '#fafcfb', borderBottom: '1px solid #f0f4f2', textAlign: 'right', fontFamily: 'DM Mono, monospace', fontSize: '12px', transition: 'background 0.15s' }}>
                                    {fmt(getAmount(filteredData, { rev_stream: stream, calendar_year: year, calendar_month: month, sub_county_id: sc }))}
                                  </td>
                                ))}
                              </tr>

                              {monthExpanded && getSubStreams(stream, year, month).map(sub => {
                                const subTotal = getAmount(filteredData, { rev_stream: stream, calendar_year: year, calendar_month: month, rev_sub_stream: sub });
                                return (
                                  /* LEVEL 4 */
                                  <tr key={sub}>
                                    ...
                                    {SUB_COUNTIES.map(sc => (
                                      <td key={sc} style={{ padding: '8px 16px', background: 'white', borderBottom: '1px solid #f0f4f2', textAlign: 'right', fontFamily: 'DM Mono, monospace', fontSize: '12px', color: 'var(--gray)' }}>
                                        {fmt(getAmount(filteredData, { rev_stream: stream, calendar_year: year, calendar_month: month, rev_sub_stream: sub, sub_county_id: sc }))}
                                      </td>
                                    ))}
                                  </tr>
                                );
